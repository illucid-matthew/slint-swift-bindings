...
public class Timer {
    ...
    public func run(after duration: UInt64, _ closure: @escaping () -> ()) {
    ...
    public func run(every period: UInt64, _ closure: @escaping () -> ()) {
        start(mode: TimerMode.Repeated, duration: period, closure: closure)
    }
    ...
    /// Initializer. Doesn't need to do anything.
    public init() { }
